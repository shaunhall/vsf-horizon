...
export async function getProduct(context: Context, params: ProductQueryVariables, customQuery: CustomQuery): Promise<Product[]> {
  ...
  if (params.sku) {
    ...
    const productData = await queryWithCookies<ProductQuery>(context, productQuery, variables, customQuery)
      .then(res => res.data);
