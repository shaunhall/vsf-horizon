...
export async function getReviews(context: Context, params: ReviewsQueryVariables, customQuery: CustomQuery): Promise<Review> {
  ...
  if (params.sku) {
    ...
    const data = await queryWithCookies<ReviewsQuery>(context, reviewsQuery, variables, customQuery).then(res => res.data);
