...
export async function getSearch(context: Context, params: SearchQueryVariables, customQuery: CustomQuery): Promise<Search> {
  const variables = {
    query: params.query,
    sort: params.sort
  ...
  const searchData: SearchQuery = await queryWithCookies(context, searchQuery, variables, customQuery)
    .then(res => res.data);
